...
describe('UsuariosService', () => {
  ...
  describe('createUsuario', () => {
    ...
    it('should create a user and personalMedico entry if type is medico', async () => {
      ...
      prisma.usuarios.create.mockResolvedValue(mockUser);
      prisma.personalMedico.create.mockResolvedValue({});
    ...
    it('should create a user and recepcionistas entry if type is recepcionista', async () => {
      ...
      prisma.usuarios.create.mockResolvedValue(mockUser);
      prisma.recepcionistas.create.mockResolvedValue({});
    ...
    it('should create a user and administradores entry if type is administrador', async () => {
      ...
      prisma.usuarios.create.mockResolvedValue(mockUser);
      prisma.administradores.create.mockResolvedValue({});
  ...
  describe('update', () => {
    it('should update a user', async () => {
      ...
      prisma.usuarios.update.mockResolvedValue(mockUser);
  ...
  describe('delete', () => {
    it('should delete user and all related records', async () => {
      prisma.usuarios.delete.mockResolvedValue({id_usuario: 1});

      await service.delete(1);

      expect(prisma.personalMedico.deleteMany).toHaveBeenCalledWith({
        where: {id_medico: 1},
      });
      expect(prisma.recepcionistas.deleteMany).toHaveBeenCalledWith({
        where: {id_recepcionista: 1},
      });
      expect(prisma.administradores.deleteMany).toHaveBeenCalledWith({
        where: {id_administrador: 1},
      });
      expect(prisma.afiliados.deleteMany).toHaveBeenCalledWith({
        where: {id_afiliado: 1},
      });
